...
class PointPillarLoss(nn.Module):
    def __init__(self, args):
        ...
        self.mc_weight = args['mc_weight'] if 'mc_weight' in args else 0
        self.after_weight = args['after_weight'] if 'after_weight' in args else 0
        self.single_weight = args['single_weight'] if 'single_weight' in args else 0

    def forward(self, output_dict, target_dict, target_dict_all=None):
        
        ##ego supervision
        total_loss = self.forward_ori(output_dict, target_dict)
        ##single car supervision
        if 'psm_mc' in output_dict and  self.mc_weight > 0:
            total_loss = total_loss + self.forward_ori(output_dict, target_dict, prefix='_mc') * self.mc_weight

        if target_dict_all is not None and 'psm_single' in output_dict and  self.single_weight > 0:
            target_dict_all, record_len = target_dict_all
            B = len(record_len)
            BL, C, H, W = output_dict['psm_single'].shape
            L = int(BL / B)
            cav_ids = []
            for b, l in enumerate(record_len):
                for i in range(l):
                    cav_ids.append(i + b*L)
            single_output_dict = {
                'rm_single': output_dict['rm_single'][cav_ids],
                'psm_single': output_dict['psm_single'][cav_ids],
            }
            single_loss = self.forward_ori(single_output_dict, target_dict_all, prefix='_single')
            total_loss = total_loss + single_loss * self.single_weight

            if 'rm_after' in output_dict and self.after_weight > 0:
                after_output_dict = {
                    'rm_after': output_dict['rm_after'][cav_ids],
                    'psm_after': output_dict['psm_after'][cav_ids],
                }
                after_loss = self.forward_ori(after_output_dict, target_dict_all, prefix='_after')
                total_loss = total_loss + after_loss * self.after_weight
        return total_loss

    def forward_ori(self, output_dict, target_dict, prefix=''):
        """
        Parameters
        ----------
        output_dict : dict
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        for k, v in self.loss_dict.items():
            if 'total_loss' in k:
                continue
            print_msg += '{}: {:.2f} | '.format(k.replace('_loss', '').replace('_single', '_s'), v.item())
