...
class VoxelPostprocessor(BasePostprocessor):
    ...
    @staticmethod
    def collate_batch(label_batch_list, is_torch=False):
        """
        ...
        if not is_torch:
            ...
            pos_equal_one = torch.cat(pos_equal_one, dim = 0)
            neg_equal_one = torch.cat(neg_equal_one, dim = 0)
    ...
    def post_process(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            # import ipdb;ipdb.set_trace()
            # liuyi
            if isinstance(cav_content, list):
                cav_content = cav_content[-1]
    ...
    @staticmethod
    def visualize(pred_box_tensor, gt_tensor, pcd, show_vis, save_path, dataset=None, align_feature=None, origin_feature=None):
        """
