...
class SplitAttn2(nn.Module):
    def __init__(self, input_dim):
        super(SplitAttn2, self).__init__()
    ...
    def forward(self, window_list):
        # window list:[(B, C, H, W) * 3]  -> [(B, L, H, W, C) * 3]
        assert len(window_list) == 3, 'only 3 windows are supported'
        window_list = window_list.permute(0, 1, 3, 4, 2).unsqueeze(2).contiguous()  # T B C H W -> T B 1 H W C
        ...
        # window list: [(B, L, H, W, C) * 3] -> [(B, C, H, W) * 3]
        return out.squeeze(1).permute(0, 3, 1, 2).contiguous()
