...
def calculate_ap(result_stat, iou, eval_type='new'):
    """
...
def eval_final_results(result_stat, save_path, range="", infer_info=None, metric="new"):
    dump_dict = {}
    file_name = 'eval.yaml' if range == "" else range+'_eval.yaml'

    # ap_30, mrec_30, mpre_30 = calculate_ap(result_stat, 0.30)
    ap_50, mrec_50, mpre_50 = calculate_ap(result_stat, 0.50, metric)
    ap_70, mrec_70, mpre_70 = calculate_ap(result_stat, 0.70, metric)
    ...
    print('The metric is %s, '
          'The range is %s, '
          'The Average Precision at IOU 0.5 is %.3f, '
          'The Average Precision at IOU 0.7 is %.3f' % (metric, range,  ap_50, ap_70))
    return range, ap_50, ap_70